...
- Consistent printing across all regions
- Professional document standards

### 4. Page Numbers

All multi-page PDFs must include page numbers in the footer:
- Format: "Page X of Y"
- Position: Bottom center or bottom right of each page
- Font size: Smaller than body text (e.g., 9pt)
- Start numbering from page 1

### 5. Typography

All PDF typography must follow the standards defined in `design_guidelines.md`:
- Use the font families specified in the design guidelines
- Follow the established font size hierarchy (headings, body, captions)
- Maintain consistent line spacing for readability
- Apply proper font weights as defined in the guidelines
- Ensure fonts are properly embedded in the PDF for consistent rendering

### 6. Document Footer

Every PDF must include a consistent footer on each page containing:
- Company name (white-label name or "OnRopePro")
- Document generation date and time (timezone-aware)
- Page numbers (as defined above)
...
All PDFs must include appropriate legal text on the final page or in the footer:
- Confidentiality notice (when applicable)
- Copyright statement with current year
- Document disclaimer (e.g., "This document was generated by OnRopePro")

Example legal footer text:
```
...
PDFs must respect the user's language preference:
- All text content must be translated based on the user's selected language
- Use the existing i18n translation system (`client/src/i18n/locales/`)
- Supported languages: English (en), French (fr), Spanish (es)
- Date/time formatting must follow locale conventions
- Number formatting must follow locale conventions (decimal separators, etc.)

The centralized PDF design function must:
1. Accept a language parameter or detect from user settings
2. Load appropriate translations for all static text
3. Format dates/numbers according to locale

### 9. Follow Design Guidelines
...
### 10. Existing PDF Branding Utility
