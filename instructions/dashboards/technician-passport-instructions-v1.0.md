...
**CRITICAL**: Profile tabs have two rendering modes that MUST stay synchronized:

| Mode | Trigger | Purpose |
|------|---------|---------|
| **View Mode** | Default state | Read-only display of profile data |
| **Edit Mode** | "Edit Profile" button | Form fields for modifying data |

**Architecture Pattern**: Both modes render from separate JSX trees within the same TabsContent. Every field must exist in BOTH trees to avoid discrepancies.

### Personal Information Tab Fields

The Personal Information tab includes these synchronized fields:

| Field | View Mode | Edit Mode | Notes |
|-------|-----------|-----------|-------|
| Email | InfoItem | Input | Read-only display vs editable |
| Phone Number | InfoItem | Input | Formatted in view mode |
| Birth Date | InfoItem | Date Input | |
| **SMS Notifications** | Interactive Switch | Form Switch | **Toggleable in BOTH modes** |
| Address | Text block | Address autocomplete | |
| Emergency Contact | Grid display | Form fields | |
| **Medical Conditions** | Text/placeholder | Textarea | **Must show in BOTH modes** |

### Interactive Fields (Work in Both Modes)

Some fields are interactive even in view mode:
- **SMS Notifications Toggle**: Users can enable/disable without entering full edit mode
- Uses inline PATCH request to update immediately

```typescript
// Example: SMS toggle in view mode
<Switch
...
- **v1.2** (December 28, 2024): Added Profile Tab Architecture section documenting view/edit mode synchronization, SMS Notifications inline toggle, and Medical Conditions field placement
