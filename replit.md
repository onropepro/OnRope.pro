...
*   **Document Review & Signature System:** Employees must review and digitally sign key company documents (Health & Safety Manual, Company Policies, Method Statements) before they are considered fully compliant. Features include:
...
*   **Company Safety Rating (CSR):** Penalty-based rating system starting at 100%. Six components: Documentation (25% penalty if missing any of Certificate of Insurance, Health & Safety Manual, or Company Policy), Toolbox Meetings (25% max penalty based on work session coverage), Harness Inspections (25% max penalty based on inspection completion), Method Statement Coverage (20% max penalty based on job type coverage), Document Reviews (5% max penalty based on unsigned employee document acknowledgments), and Project Completion (informational only, no penalty). Toolbox meetings use a 7-day coverage window - a meeting covers work sessions within 7 days in either direction, allowing meetings to occur outside of work sessions and still count toward compliance. Property managers can view vendor CSR scores with detailed breakdowns.
