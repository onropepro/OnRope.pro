...
*   **Safety & Compliance:** Digitizes harness inspections, toolbox meeting documentation, and gear inventory management with date of manufacture tracking.
...
*   **White Label Branding System:** Subscription-gated feature ($0.49/month) allowing companies to customize the entire platform with custom logos and unlimited brand colors. When active, branding applies globally across all pages via CSS variables (--brand-primary), with utility classes (.custom-brand-text, .custom-brand-border, .custom-brand-bg, .gradient-text). Implements proper error handling to distinguish missing branding (404) from API failures, with automatic cleanup when branding changes.
