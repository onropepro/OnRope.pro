...
*   **Technician Self-Registration:** Comprehensive multi-step registration wizard collecting: name, certification (IRATA/SPRAT with split level/number format), address (street, city, province, country, postal code), email, phone, password, emergency contact, SIN (optional), bank info (transit, institution, account numbers) with void cheque upload, driver's license with expiry and abstract upload, birthday, medical conditions, and rope access experience start date. Includes clear privacy notice explaining data is used only by employer for HR/payroll/driving privileges and is never shared externally.
*   **Experience Tracking:** Technicians can record when they started their rope access career during registration. Experience is calculated and displayed in years/months format on both the Technician Portal and Dashboard Employee Details. Uses timezone-safe date handling via parseLocalDate utility.
*   **Technician Referral System:** Each technician receives a unique 10-character referral code upon registration. Technicians can share their code with other rope access workers, and new registrants can enter a referral code during registration. The system tracks who referred whom (referredByUserId, referredByCode) and displays referral counts in both the Technician Portal and SuperUser dashboard. Codes use a limited character set (no 0/O/1/I) for easy sharing. Bilingual support (EN/FR).
*   **Resume/CV Upload:** Technicians can upload multiple resume/CV documents through the Technician Portal. Supports PDF, image, and document formats with preview display.
...
*   **Global Building Database:** SuperUser-managed centralized database of all buildings across all companies. Buildings are auto-created when projects are created with strata plan numbers. Each building has login credentials (strata number = username, password defaults to strata number) for a building portal that shows cross-company maintenance history. Includes map-based search with Leaflet integration and Geoapify geocoding for building locations.
*   **Address Autocomplete:** Project creation form uses Geoapify-powered address autocomplete (`AddressAutocomplete` component) to ensure complete addresses with city, province, country, and postal code. This enables accurate geocoding for map display. The component supports both manual typing and suggestion selection.
