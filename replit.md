...
*   **Job Board System (In Progress):** Companies and SuperUsers can post employment opportunities. Technicians can browse jobs and opt-in to make their profile visible to employers. Visible profile includes: resume, safety rating, name, years of experience, IRATA/SPRAT cert numbers, and photo. Schema: `job_postings` table with company_id, title, description, requirements, location, job_type, employment_type, salary range, required certifications, status. Users table has `isVisibleToEmployers` and `visibilityEnabledAt` fields.
