...
*   **Pre-Launch Landing Page System:** Temporary "gating" pages for homepage, /technician, and /property-manager until January 2026 launch. Features: 4-digit PIN bypass (2026) in footer, early access signup forms, backend storage in `early_access_signups` table. PIN stored in localStorage (`onropepro_prelaunch_bypass`). Set `isPreLaunchActive` in PreLaunchContext.tsx to `false` to disable.
