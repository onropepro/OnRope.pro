...
- **License Key Verification System:** Subscription-based license verification integrated with Overhaul Labs Marketplace API. Company role users require valid license verification to access full platform functionality. System features: (1) Auto re-verification on every login with graceful API failure handling that preserves existing status during outages, (2) Manual verification endpoint (POST `/api/verify-license`) with bypass mode for development, (3) Frontend verification gate that redirects unverified companies to `/license-verification` page, (4) Database fields (`licenseKey`, `licenseVerified`) with security enforcement stripping `licenseKey` from ALL API responses, (5) Bypass mode uses `licenseKey: "BYPASSED"` and persists across re-logins by skipping re-verification, (6) Only company role affected - employees operate normally without license checks, (7) Read-only permission helper (`isReadOnly()`) ready for future UI enforcement. Extensively tested with 100% pass rate across 17 test phases covering security, persistence, edge cases, stress tests, and complete end-to-end cycles.
