...
export class Storage {
  ...
  async createEarlyAccessSignup(data: InsertEarlyAccessSignup): Promise<EarlyAccessSignup> {
    const result = await db.insert(earlyAccessSignups).values(data).returning();
    return result[0];
  }

  async getEarlyAccessSignups(): Promise<EarlyAccessSignup[]> {
    return await db.select().from(earlyAccessSignups).orderBy(desc(earlyAccessSignups.createdAt));
  }
}
