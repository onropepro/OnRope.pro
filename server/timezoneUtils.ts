...
export function getTodayStartInTimezone(timezone: string): Date {
  return getZonedDayBounds(timezone).startOfDay;
}

export function getTomorrowStartInTimezone(timezone: string): Date {
  const { endOfDay } = getZonedDayBounds(timezone);
  return new Date(endOfDay.getTime() + 1);
}

export function sessionOverlapsDay(
  sessionStart: Date,
  sessionEnd: Date | null,
  timezone: string,
  referenceDate?: Date
): boolean {
  const { startOfDay, endOfDay } = getZonedDayBounds(timezone, referenceDate);
  const tomorrowStart = new Date(endOfDay.getTime() + 1);
  
  const effectiveEnd = sessionEnd || new Date();
  
  return sessionStart < tomorrowStart && effectiveEnd > startOfDay;
}
