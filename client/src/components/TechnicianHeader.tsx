    ...
    <header className="sticky top-0 z-[100] h-14 bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-700/80 px-4 sm:px-6">
      <div className="h-full flex items-center justify-between gap-4">
        <div className="flex items-center gap-4 flex-1 min-w-0">
          {/* Mobile button: hamburger menu if callback provided, otherwise home button */}
          <Button
            variant="ghost"
            size="icon"
            onClick={onMobileMenuClick ? onMobileMenuClick : () => setLocation('/technician-portal')}
            className="lg:hidden text-slate-600 dark:text-slate-300"
            data-testid={onMobileMenuClick ? "button-mobile-menu" : "button-back-home"}
          >
            {onMobileMenuClick ? <Menu className="h-5 w-5" /> : <Home className="w-5 h-5" />}
          </Button>
